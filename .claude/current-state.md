...
- **PR 3** `1e90f38a299dc7a5f4a60885c8464b25e1098164` — Runtime capability resolution. New `server/app/site-platform/runtime-capabilities.mjs` (`publishedCapabilityContext`, `draftCapabilityContext`, `commerceEnabled`; imports only the resolver and manifest reader). Both public HTML routes — `createPublicSitesRouter.productsFor` (`public-sites.mjs`) and `sendLegacySite()` (`auth.mjs`, the only function changed there) — decide product loading through `commerceEnabled(project)`. Commerce remains STORE-only: the decision re-resolves from the project's current `siteType`, never from `manifest.capabilities`. `public-site-server.mjs` no longer creates the ecommerce service at startup; `createPublicSitesRouter` takes a `createEcommerceService` factory, creates the service on the first STORE request and reuses it (static import, no dynamic import). Error behavior unchanged: `public-sites` catalog or creation failure ⇒ `[]`; `sendLegacySite` still returns 500. Auth JSON APIs (`publicStore`, `publicSite`, storefront/cart/checkout/payment routes, SQL `site_type` filters) untouched. Renderers remain capability-unaware; snapshots unchanged (byte-identical). The Phase 1 guard test was deliberately narrowed: routers may import only `runtime-capabilities.mjs`. Tests: new `site-platform-runtime-capabilities.test.mjs` 7/7; server suite 1118/1118.
...
**Commerce rule:** commerce/payments resolve for `STORE` only. Renderers still dispatch by `siteType` (`isCorporateV16` / `isStoreV16`) and import nothing from `site-platform/`. Since PR 2 the publish path (`site-project-repository.mjs`) uses `publish-manifest.mjs`; since PR 3 the public routers (`public-sites.mjs`, `auth.mjs` `sendLegacySite`) use `runtime-capabilities.mjs` only (enforced by an agreement test).

**Tests:** `node --test test/site-platform*` 47/47; server suite 1118/1118.
...
**Next:** not yet chosen. Candidates from ADR-004: section-registry renderer dispatch (phase 4, baselines must stay byte-identical), neutral core presentation (phase 5), or confirming/fixing the standalone preview workspace-context issue. Changing the STORE-only commerce rule requires a separate ADR.
