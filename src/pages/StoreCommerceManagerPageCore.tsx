...
type Order={id:string;email?:string|null;currency:string;status:string;paymentStatus:string;fulfillmentStatus:string;paymentProvider?:string|null;paymentReference?:string|null;shippingMethod?:string|null;shippingAddress?:ShippingAddress|null;subtotalMinor:number;discountMinor:number;shippingMinor:number;totalMinor:number;items:OrderItem[];createdAt:string};
...
export default function StoreCommerceManagerPage(){
 ...
 {view==="orders"&&<OrdersView orders={orders} selectedOrderId={selectedOrderId} onSelect={setSelectedOrderId} busy={orderBusy} message={orderMessage} onUpdateStatus={updateOrderStatus} onPaymentChanged={()=>void refresh()}/>}
...
function OrdersView({orders,selectedOrderId,onSelect,busy,message,onUpdateStatus,onPaymentChanged}:{orders:Order[];selectedOrderId:string|null;onSelect:(id:string|null)=>void;busy:boolean;message:string;onUpdateStatus:(id:string,patch:{status?:string;fulfillmentStatus?:string})=>void;onPaymentChanged:()=>void}){
 return <section className="rounded-2xl border border-white/10 bg-[#0d1320] p-5">
  ...
  {orders.length?<div className="space-y-2">{orders.map(o=><div key={o.id} className="rounded-xl border border-white/10 bg-white/[.025] p-4">
    ...
    {selectedOrderId===o.id&&<div className="mt-4 border-t border-white/10 pt-4">
      ...
      <PaymentAttemptsPanel order={o} onChanged={onPaymentChanged}/>
    </div>}
...
type PaymentAttempt={id:string;provider:string;status:string;amountMinor:number;currency:string;providerTransactionId:string|null;verificationCode:string|null;createdAt:string};
const ATTEMPT_STATUS_LABELS:Record<string,string>={CREATED:"ایجاد شده",REDIRECT_READY:"منتظر پرداخت مشتری",PENDING_VERIFICATION:"در انتظار تأیید",SUCCEEDED:"موفق",FAILED:"ناموفق",CANCELLED:"لغو شده",RECONCILIATION_REQUIRED:"نیاز به بررسی دستی"};
const OPEN_ATTEMPT=new Set(["REDIRECT_READY","PENDING_VERIFICATION","RECONCILIATION_REQUIRED"]);
// P1b: read-only view of an order's gateway attempts, plus a server-side re-check of an open one.
// Re-check never marks anything paid on its own; the server verifies with the gateway first.
function PaymentAttemptsPanel({order,onChanged}:{order:Order;onChanged:()=>void}){
 const[attempts,setAttempts]=useState<PaymentAttempt[]|null>(null),[busyId,setBusyId]=useState<string|null>(null),[note,setNote]=useState("");
 async function load(){try{const d=await read(await apiFetch(`/api/commerce/orders/${order.id}/payment-attempts`));setAttempts(d.attempts||[])}catch(e){setAttempts([]);setNote(e instanceof Error?e.message:"خطا در دریافت پرداخت‌ها")}}
 useEffect(()=>{void load()},[order.id,order.paymentStatus]);
 async function recheck(id:string){setBusyId(id);setNote("");try{const d=await read(await apiFetch(`/api/commerce/payment-attempts/${id}/reconcile`,{method:"POST"}));setNote(d.result==="paid"?"پرداخت تأیید و سفارش پرداخت‌شده ثبت شد.":d.result==="failed"?"درگاه این پرداخت را ناموفق اعلام کرد.":"نتیجه هنوز مشخص نیست؛ کمی بعد دوباره بررسی کنید.");await load();onChanged()}catch(e){setNote(e instanceof Error?e.message:"بررسی مجدد ناموفق بود")}finally{setBusyId(null)}}
 if(attempts===null)return null;
 return <div className="mt-3 rounded-xl border border-white/10 bg-white/[.025] p-3">
  <b className="text-xs">پرداخت‌های آنلاین</b>
  {order.paymentReference&&<p className="mt-1 text-[11px] text-white/50">کد پیگیری درگاه: <b dir="ltr">{order.paymentReference}</b></p>}
  {attempts.some(a=>a.status==="RECONCILIATION_REQUIRED")&&<p className="mt-2 rounded-lg border border-rose-500/30 bg-rose-500/10 p-2 text-[11px] text-rose-200">درگاه پرداخت را تأیید کرده ولی سفارش پرداخت‌شده ثبت نشد. پیش از هر اقدامی «بررسی مجدد» را بزنید و در صورت تکرار با پشتیبانی تماس بگیرید.</p>}
  {attempts.length?<div className="mt-2 space-y-2">{attempts.map(a=><div key={a.id} className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-white/60">
   <span>{a.provider} · {new Date(a.createdAt).toLocaleString("fa-IR")} · <b className={a.status==="SUCCEEDED"?"text-emerald-300":a.status==="RECONCILIATION_REQUIRED"?"text-rose-300":"text-white/70"}>{ATTEMPT_STATUS_LABELS[a.status]||a.status}</b>{a.verificationCode&&<span dir="ltr" className="text-white/30"> · {a.verificationCode}</span>}</span>
   {OPEN_ATTEMPT.has(a.status)&&<button type="button" disabled={busyId!==null} onClick={()=>void recheck(a.id)} className="rounded-lg border border-white/15 px-2 py-1 disabled:opacity-50">{busyId===a.id?"در حال بررسی…":"بررسی مجدد"}</button>}
  </div>)}</div>:<p className="mt-2 text-[11px] text-white/35">پرداخت آنلاینی برای این سفارش ثبت نشده است.</p>}
