...
export default function GrowthLoopPage(){
  ...
  return <main dir="rtl" className="min-h-screen bg-[#05060a] text-white"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
    <header className="min-w-0 rounded-3xl border border-white/10 bg-gradient-to-bl from-violet-500/15 to-cyan-400/5 p-5 sm:p-8"><Link to="/dashboard" className="inline-flex min-h-11 items-center gap-2 text-sm text-white/60"><ArrowRight/> داشبورد</Link><p className="mt-5 text-xs font-bold tracking-widest text-violet-300">GROWTH LOOP · COPILOT</p><h1 className="mt-2 text-2xl font-black sm:text-4xl">چرخهٔ رشد قابل توضیح</h1><p className="mt-3 max-w-3xl leading-8 text-white/55">از هدف و محتوای تأییدشده تا شواهد واقعی CRM و تصمیم انسانی؛ بدون انتشار، هزینه یا اجرای خودکار.</p></header>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2">
      ...
      <Card n="۴" title="شواهد CRM"><button disabled={!candidate||busy!==""} onClick={runCopilot} className="min-h-11 rounded-xl bg-white/10 px-4 text-sm disabled:opacity-40">خواندن شواهد موجود</button><div className="mt-3 flex min-w-0 flex-col gap-2 sm:flex-row"><input aria-label="شناسه لید" value={leadId} onChange={e=>setLeadId(e.target.value)} className="min-h-11 min-w-0 w-full flex-1 rounded-xl border border-white/10 bg-black/30 px-3" placeholder="شناسه لید واقعی"/><button disabled={!candidate||!leadId||busy!==""} onClick={convert} className="min-h-11 w-full rounded-xl bg-cyan-500 px-4 font-bold text-black disabled:opacity-40 sm:w-auto">تبدیل canonical</button></div><Fact label="وضعیت evidence" value={copilot?.result.evidence?.state||"UNKNOWN"}/><Fact label="تعداد مراجع" value={String(copilot?.result.evidence?.references?.length||0)}/></Card>
