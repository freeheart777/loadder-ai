...
export default function GrowthLoopPage(){
  ...
  return <main dir="rtl" className="min-h-screen bg-[#05060a] text-white"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2">
      <Card n="۱" title="هدف و زمینه"><Fact label="هدف" value={describeGrowthGoal(experiment.goalRef)}/><Fact label="معیار" value={`${fa(experiment.goalContract.metric)} · هدف ${experiment.goalContract.target}`}/><Fact label="زمینهٔ کسب‌وکار" value={describeGrowthContext(experiment.goalContextVersionId)}/><Fact label="خط مبنا" value={fa(experiment.goalContract.baseline.state)}/></Card>
      <Card n="۲" title="آزمایش"><Fact label="فرضیه" value={experiment.hypothesis}/><Fact label="درمان" value={experiment.treatmentDefinition}/><Fact label="پنجره" value={`${formatGrowthWindowDate(experiment.goalContract.measurementWindow.start)} ← ${formatGrowthWindowDate(experiment.goalContract.measurementWindow.end)}`}/></Card>
