...
const fa=(value:string)=>copy[value]||value.replaceAll("_"," ");

export default function GrowthLoopPage(){
  const {experimentId=""}=useParams();
  const [experiment,setExperiment]=useState<Experiment|null>(null),[briefs,setBriefs]=useState<Brief[]>([]),[candidate,setCandidate]=useState<Candidate|null>(null);
  ...
  const load=useCallback(async()=>{
    ...
    const b=await request<{result:Brief[]}>(`/api/growth/experiments/${encodeURIComponent(experimentId)}/briefs?page=1&pageSize=25`); setBriefs(b.result||[]);
    ...
    setCandidate(candidates.find(x=>x.state==="APPROVED")||candidates[0]||null);
  },[experimentId]);
  ...
  return <main dir="rtl" className="min-h-screen bg-[#05060a] text-white"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
    <header className="rounded-3xl border border-white/10 bg-gradient-to-bl from-violet-500/15 to-cyan-400/5 p-5 sm:p-8"><Link to="/dashboard" className="inline-flex min-h-11 items-center gap-2 text-sm text-white/60"><ArrowRight/> داشبورد</Link><p className="mt-5 text-xs font-bold tracking-widest text-violet-300">GROWTH LOOP · COPILOT</p><h1 className="mt-2 text-2xl font-black sm:text-4xl">چرخهٔ رشد قابل توضیح</h1><p className="mt-3 max-w-3xl leading-8 text-white/55">از هدف و محتوای تأییدشده تا شواهد واقعی CRM و تصمیم انسانی؛ بدون انتشار، هزینه یا اجرای خودکار.</p></header>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2">
      <Card n="۱" title="هدف و زمینه"><Fact label="هدف" value={experiment.goalRef}/><Fact label="معیار" value={`${experiment.goalContract.metric} · ${experiment.goalContract.direction} · ${experiment.goalContract.target} ${experiment.goalContract.unit}`}/><Fact label="زمینهٔ پین‌شده" value={contextId}/><Fact label="خط مبنا" value={experiment.goalContract.baseline.state}/></Card>
      ...
      <Card n="۳" title="محتوای درمان"><Fact label="Brief" value={briefs[0]?.id||"UNKNOWN"}/><Fact label="Candidate" value={candidate?.id||"UNKNOWN"}/><Fact label="وضعیت" value={candidate?.state||"UNKNOWN"}/><p className="mt-3 text-xs text-white/40">فقط Candidate تأییدشده هویت درمان است؛ تأیید محتوا مجوز انتشار نیست.</p></Card>
      <Card n="۴" title="شواهد CRM"><button disabled={!candidate||busy!==""} onClick={runCopilot} className="min-h-11 rounded-xl bg-white/10 px-4 text-sm disabled:opacity-40">خواندن شواهد موجود</button><div className="mt-3 flex gap-2"><input aria-label="شناسه لید" value={leadId} onChange={e=>setLeadId(e.target.value)} className="min-h-11 min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-3" placeholder="شناسه لید واقعی"/><button disabled={!candidate||!leadId||busy!==""} onClick={convert} className="min-h-11 rounded-xl bg-cyan-500 px-4 font-bold text-black disabled:opacity-40">تبدیل canonical</button></div><Fact label="وضعیت evidence" value={copilot?.result.evidence?.state||"UNKNOWN"}/><Fact label="تعداد مراجع" value={String(copilot?.result.evidence?.references?.length||0)}/></Card>
      <Card n="۵" title="ارزیابی"><button disabled={!candidate||busy!==""} onClick={assess} className="min-h-11 rounded-xl bg-violet-500 px-4 font-bold disabled:opacity-40">اجرای سیاست deterministic</button><Fact label="نتیجه" value={finding?fa(finding.state):"UNKNOWN"}/><Fact label="شمارش مطلق" value={finding?String(finding.value.observedCount):"UNKNOWN"}/>{reasons.map(x=><p key={x} className="mt-2 text-sm text-amber-200">• {fa(x)}</p>)}<p className="mt-3 text-xs leading-6 text-white/40">ACTIONABLE فقط یعنی شواهد برای بازبینی محدود انسانی کافی است؛ نه موفقیت، uplift، درآمد یا علیت.</p></Card>
      <Card n="۶" title="پیشنهاد و تصمیم انسانی"><Fact label="پیشنهاد" value={recommendation?fa(recommendation.considerationCode):"UNKNOWN"}/><Fact label="نیازمند" value={recommendation?.provenance.requiredApproval||"HUMAN_REVIEW"}/><button disabled={!recommendation||Boolean(decision)||busy!==""} onClick={adopt} className="mt-3 min-h-11 rounded-xl bg-emerald-500 px-4 font-bold text-black disabled:opacity-40">پذیرش برای بررسی</button>{decision&&<div className="mt-3 rounded-xl border border-emerald-400/20 bg-emerald-500/10 p-3 text-sm text-emerald-200"><CheckCircle className="ml-2 inline"/>پیشنهاد پذیرفته شد؛ هیچ اقدام خارجی هنوز اجرا نشده است.</div>}</Card>
    </div><footer className="mt-5 rounded-2xl border border-white/10 p-4 text-xs leading-6 text-white/40">Execution authorized: FALSE · Ads spend: NONE · Publishing: NONE · Financial mutation: NONE</footer>
  </div></main>;
}
function Card({n,title,children}:{n:string;title:string;children:ReactNode}){return <section className="rounded-3xl border border-white/10 bg-white/[.035] p-5"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-violet-500/15 text-violet-300">{n}</span><h2 className="font-bold">{title}</h2></div><div className="mt-4">{children}</div></section>}
function Fact({label,value}:{label:string;value:string}){return <div className="mt-2 flex min-h-8 items-start justify-between gap-4 text-sm"><span className="text-white/40">{label}</span><span className="break-all text-left text-white/75">{value}</span></div>}
