  ...
  return <main dir="rtl" className="loadder-dashboard-bg min-h-screen text-white">
    ...
    <div className="mx-auto max-w-[1400px] px-6 py-8">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        ...
        <aside className="rounded-[30px] border border-violet-400/15 bg-[#080d1d]/70 p-7"><div className="flex items-center justify-between"><div><h2 className="text-xl font-semibold">۳. نسخه‌های تولیدشده</h2><p className="mt-1 text-sm text-white/40">نسخه را انتخاب، کپی یا ذخیره کن.</p></div><button type="button" disabled={!currentText} onClick={async () => { await navigator.clipboard.writeText(currentText); setNotice("نسخه انتخاب‌شده کپی شد."); }} className="rounded-xl border border-white/10 bg-white/5 p-3 disabled:opacity-30"><Copy size={18}/></button></div>{generation ? <><div className="mt-5 flex gap-2">{generation.variants.map((_, index) => <button key={index} type="button" onClick={() => { setSelectedVariant(index); setSavedItemId(null); }} className={`rounded-xl px-4 py-2 text-sm ${index === selectedVariant ? "bg-violet-500 text-white" : "bg-white/5 text-white/50"}`}>نسخه {index + 1}</button>)}</div><pre dir="rtl" className="mt-4 min-h-[300px] whitespace-pre-wrap rounded-2xl border border-white/10 bg-black/20 p-5 font-sans text-sm leading-8 text-white/75">{currentText}</pre><div className="mt-4 grid grid-cols-2 gap-3"><button type="button" onClick={saveVariant} className="flex items-center justify-center gap-2 rounded-xl bg-cyan-500/15 px-4 py-3 text-sm text-cyan-200"><FloppyDisk/>ذخیره در کتابخانه</button><button type="button" disabled={loading} onClick={() => generate(true)} className="rounded-xl border border-violet-400/25 bg-violet-500/10 px-4 py-3 text-sm">تولید نسخه‌های تازه</button></div>{savedItemId && <Link to={`/dashboard/library/${savedItemId}`} className="mt-3 block text-center text-sm text-violet-300">باز کردن در کتابخانه</Link>}</> : <div className="mt-5 flex min-h-[360px] items-center justify-center rounded-2xl border border-dashed border-white/10 bg-black/10 p-8 text-center text-sm leading-8 text-white/35">پس از تکمیل بریف، خروجی ساختاریافته و برندمحور اینجا نمایش داده می‌شود.</div>}{notice && <div className="mt-4 text-sm text-cyan-300">{notice}</div>}</aside>
      </section>
    </div>
  </main>;
}
