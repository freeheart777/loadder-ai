...
import { ArrowRight, Copy, FileText, FloppyDisk, InstagramLogo, MagicWand, Megaphone, Sparkle } from "@phosphor-icons/react";
...
type CreativeIntent = { id: string; title: string; intentType: string };
...
export default function ContentStudioPage() {
  ...
  const [intents, setIntents] = useState<CreativeIntent[]>([]);
  ...
  useEffect(() => { void apiFetch("/api/creative-intents?limit=100").then(async response => response.ok ? response.json() : null).then(data => setIntents(data?.intents || [])).catch(() => undefined); }, []);
  ...
  async function saveVariant() {
    ...
    setError(""); setNotice("");
    const response = await apiFetch(`/api/content/generations/${generation.generationId}/save`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ variantIndex: selectedVariant }) });
    const data = await response.json();
    if (!response.ok) { setError("ذخیره در کتابخانه انجام نشد."); return; }
    setSavedItemId(data.item.id); setNotice("نسخه انتخاب‌شده در کتابخانه ذخیره شد.");
  }
  ...
  return <main dir="rtl" className="loadder-dashboard-bg min-h-screen text-white">
    ...
    <div className="mx-auto max-w-[1400px] px-6 py-8">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        <div className="rounded-[30px] border border-white/10 bg-[#080d1d]/70 p-7"><h2 className="text-xl font-semibold">۲. بریف محتوا</h2><label className="mt-5 mb-2 block text-sm text-white/55">هدف این محتوا چیست؟</label><select value={intentId} onChange={event => { setIntentId(event.target.value); setRequestKey(null); }} className="w-full rounded-2xl border border-white/10 bg-[#080d1d] px-4 py-3 text-sm outline-none"><option value="">بدون هدف ثبت‌شده (سازگاری قبلی)</option>{intents.map(intent => <option key={intent.id} value={intent.id}>{intent.title}</option>)}</select><div className="mt-5 grid gap-4 md:grid-cols-2"><Field label="هدف بریف" value={goal} onChange={setGoal} maxLength={300}/><Field label="پیشنهاد / محصول / خدمت *" value={offering} onChange={setOffering} maxLength={500}/><Field label="مخاطب دقیق‌تر" value={audience} onChange={setAudience} maxLength={500}/><Field label="پیام کلیدی *" value={keyMessage} onChange={setKeyMessage} maxLength={500}/><Field label={`دعوت به اقدام${selected.requiresCta ? " *" : ""}`} value={cta} onChange={setCta} maxLength={160}/><Field label="لحن تکمیلی" value={tone} onChange={setTone} maxLength={200}/></div><div className="mt-4"><label className="mb-2 block text-sm text-white/55">متن مرجع اختیاری</label><textarea value={referenceText} onChange={(event) => setReferenceText(event.target.value)} maxLength={2000} rows={4} className="w-full resize-none rounded-2xl border border-white/10 bg-black/20 p-4 text-sm leading-7 outline-none focus:border-violet-400/40"/></div>{error && <div className="mt-4 rounded-2xl border border-red-400/20 bg-red-500/10 px-4 py-3 text-sm text-red-200">{error}</div>}<button type="button" disabled={loading} onClick={() => generate(false)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-violet-600 via-blue-600 to-fuchsia-500 px-6 py-4 font-semibold disabled:opacity-50"><Sparkle size={18} weight="fill"/>{loading ? "در حال تولید متن…" : "تولید ۳ نسخه"}</button><p className="mt-3 text-xs leading-6 text-white/35">این بخش فقط محتوا تولید می‌کند؛ هیچ پیام، تبلیغ یا کمپینی منتشر یا اجرا نمی‌شود.</p></div>
        <aside className="rounded-[30px] border border-violet-400/15 bg-[#080d1d]/70 p-7"><div className="flex items-center justify-between"><div><h2 className="text-xl font-semibold">۳. نسخه‌های تولیدشده</h2><p className="mt-1 text-sm text-white/40">نسخه را انتخاب، کپی یا ذخیره کن.</p></div><button type="button" disabled={!currentText} onClick={async () => { await navigator.clipboard.writeText(currentText); setNotice("نسخه انتخاب‌شده کپی شد."); }} className="rounded-xl border border-white/10 bg-white/5 p-3 disabled:opacity-30"><Copy size={18}/></button></div>{generation ? <><div className="mt-5 flex gap-2">{generation.variants.map((_, index) => <button key={index} type="button" onClick={() => { setSelectedVariant(index); setSavedItemId(null); }} className={`rounded-xl px-4 py-2 text-sm ${index === selectedVariant ? "bg-violet-500 text-white" : "bg-white/5 text-white/50"}`}>نسخه {index + 1}</button>)}</div><pre dir="rtl" className="mt-4 min-h-[300px] whitespace-pre-wrap rounded-2xl border border-white/10 bg-black/20 p-5 font-sans text-sm leading-8 text-white/75">{currentText}</pre><div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2"><button type="button" onClick={saveVariant} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-cyan-500/15 px-4 py-3 text-sm text-cyan-200"><FloppyDisk/>ذخیره در کتابخانه</button><button type="button" disabled={loading} onClick={() => generate(true)} className="min-h-11 rounded-xl border border-violet-400/25 bg-violet-500/10 px-4 py-3 text-sm">تولید نسخه‌های تازه</button></div>{savedItemId && <section aria-labelledby="content-next-steps-title" className="mt-5 rounded-2xl border border-cyan-400/20 bg-cyan-500/[0.06] p-4"><h3 id="content-next-steps-title" className="font-semibold text-white">محتوای شما آماده است؛ مرحله بعد چیست؟</h3><div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2"><Link to={`/dashboard/library/${savedItemId}`} className="flex min-h-11 items-center justify-center rounded-xl bg-violet-500/15 px-4 py-3 text-center text-sm text-violet-200">مشاهده در کتابخانه</Link><Link to="/dashboard/landings/new" className="flex min-h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-center text-sm text-white/75">ساخت صفحه فرود با این محتوا</Link><Link to="/dashboard/websites/new" className="flex min-h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-center text-sm text-white/75">ساخت سایت</Link><Link to="/dashboard" className="flex min-h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-center text-sm text-white/75">بازگشت به داشبورد</Link></div><p className="mt-4 text-xs leading-6 text-white/40">این انتخاب‌ها فقط شما را به مرحله بعد می‌برند؛ هیچ محتوا، صفحه یا سایتی خودکار ساخته، منتشر یا اجرا نمی‌شود.</p></section>}</> : <div className="mt-5 flex min-h-[360px] items-center justify-center rounded-2xl border border-dashed border-white/10 bg-black/10 p-8 text-center text-sm leading-8 text-white/35">پس از تکمیل بریف، خروجی ساختاریافته و برندمحور اینجا نمایش داده می‌شود.</div>}{notice && <div className="mt-4 text-sm text-cyan-300">{notice}</div>}</aside>
      </section>
    </div>
  </main>;
}
