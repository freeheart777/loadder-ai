...
export default function CommercePaymentPage() {
  ...
  const load = useCallback(async () => { try { const result = await call(`/api/public/commerce/payments/${encodeURIComponent(attemptId)}/status`); setAttempt(result.attempt); setOrder(result.order || null); setFulfillment(result.fulfillment || null); setMessage(result.attempt.status === "VERIFIED" ? "پرداخت موفق" : result.attempt.status === "FAILED" ? "پرداخت ناموفق" : "وضعیت پرداخت نامشخص / دوباره بررسی شود"); } catch { setMessage("وضعیت پرداخت نامشخص / دوباره بررسی شود"); } }, [attemptId]);
