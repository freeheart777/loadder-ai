...
function EditorGroup({ title, hint, children }: { title: string; hint: string; children: React.ReactNode }) {
  return <section className="space-y-3 rounded-2xl border border-white/10 bg-white/[.025] p-3"><div><b className="text-xs text-white/85">{title}</b><p className="mt-1 text-[10px] leading-5 text-white/35">{hint}</p></div>{children}</section>;
}
...
function HeroEditor({ config, assets, actions }: Props) {
  return <div className="space-y-4"><PanelTitle eyebrow="HERO" title="ویترین اصلی" text="متن، تصویر و ترکیب‌بندی Hero به‌صورت زنده به‌روزرسانی می‌شود."/>
    <EditorGroup title="محتوا" hint="پیام، تصویر و فراخوان اصلی این بخش."><div className="rounded-xl border border-white/10 bg-white/[.03] p-3"><b className="text-xs">چیدمان‌های آماده</b><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => actions.patchHero({ layout: "split", alignment: "right", height: 480 })} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold hover:border-emerald-400/40">بنر بزرگ + متن</button><button type="button" onClick={() => actions.patchHero({ layout: "background", alignment: "right", height: 520, overlayOpacity: 45 })} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold hover:border-emerald-400/40">بنر تمام‌عرض</button><button type="button" onClick={() => actions.patchHero({ layout: "centered", alignment: "center", height: 440 })} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold hover:border-emerald-400/40">کمپین مرکزی</button><button type="button" onClick={() => actions.patchHero({ layout: "minimal", alignment: "right", height: 340 })} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold hover:border-emerald-400/40">مینیمال سبک</button></div></div><Field label="Eyebrow" value={config.hero.eyebrow} onChange={(eyebrow) => actions.patchHero({ eyebrow })}/><Field label="عنوان" value={config.hero.title} onChange={(title) => actions.patchHero({ title })}/><TextArea label="زیرعنوان" value={config.hero.subtitle} onChange={(subtitle) => actions.patchHero({ subtitle })}/><div className="grid grid-cols-2 gap-3"><Field label="متن CTA" value={config.hero.ctaLabel} onChange={(ctaLabel) => actions.patchHero({ ctaLabel })}/><Field label="مقصد CTA" value={config.hero.ctaHref} onChange={(ctaHref) => actions.patchHero({ ctaHref })}/></div><MediaSelect label="تصویر از Media Library" value={config.hero.imageUrl} assets={assets} onChange={(imageUrl) => actions.patchHero({ imageUrl })}/><Field label="یا URL تصویر" value={config.hero.imageUrl} onChange={(imageUrl) => actions.patchHero({ imageUrl })}/></EditorGroup>
    <EditorGroup title="طراحی" hint="چیدمان، رنگ و خوانایی روی صفحه."><Toggle label="نمایش Hero" checked={config.hero.enabled} onChange={(enabled) => actions.patchHero({ enabled })}/><Select label="چیدمان" value={config.hero.layout} onChange={(layout) => actions.patchHero({ layout: layout as typeof config.hero.layout })}><option value="centered">مرکزی</option><option value="split">متن و تصویر</option><option value="background">تصویر پس‌زمینه</option><option value="minimal">مینیمال</option></Select><div className="grid grid-cols-2 gap-2"><Color label="پس‌زمینه" value={config.hero.backgroundColor} onChange={(backgroundColor) => actions.patchHero({ backgroundColor })}/><Color label="متن" value={config.hero.textColor} onChange={(textColor) => actions.patchHero({ textColor })}/></div><Select label="تراز متن" value={config.hero.alignment} onChange={(alignment) => actions.patchHero({ alignment: alignment as typeof config.hero.alignment })}><option value="right">راست</option><option value="center">مرکز</option><option value="left">چپ</option></Select></EditorGroup>
    <EditorGroup title="پیشرفته" hint="تنظیمات فنی نمایش در قاب Hero."><Range label="ارتفاع" value={config.hero.height} min={280} max={720} onChange={(height) => actions.patchHero({ height })}/><Range label="Overlay" value={config.hero.overlayOpacity} min={0} max={90} onChange={(overlayOpacity) => actions.patchHero({ overlayOpacity })}/></EditorGroup>
  </div>;
}
