...
export default function PublicStorefrontRuntime({ page }: { page: "storefront" | "product" }) {
  ...
  useEffect(() => { if (!siteProjectId) return; const controller = new AbortController(); void (async () => { try {
    ...
    setCart(await getPublicCart(siteProjectId));
  ...
  const adapter = {
    openStorefront: () => navigate(base),
    openCollection: () => navigate(`${base}#store-products`),
    openProduct: (product: Product) => navigate(`${base}/product/${encodeURIComponent(product.slug || product.id)}`),
    openCart: () => navigate(`${base}/cart`),
    addProduct: async (product: Product) => {
      try {
        setNotice("");
        const variant = (product.variants || []).find((item) => item.id && (item.purchasable ?? (item.inventoryPolicy !== "DENY" || item.inventoryQuantity > 0)));
        if (!variant?.id) throw new Error("این محصول اکنون قابل سفارش نیست.");
        await addPublicCartItem(storeId, product.currency, variant.id, 1);
        setCart(await getPublicCart(storeId));
        setNotice("به سبد خرید اضافه شد.");
      } catch (error) {
        setNotice(error instanceof Error ? error.message : "افزودن به سبد خرید ناموفق بود.");
      }
    },
    cartCount,
    cart,
    checkout: async (input: PublicCheckoutInput) => {
      const { order } = await checkoutPublicCart(storeId, input);
      setCart(null);
