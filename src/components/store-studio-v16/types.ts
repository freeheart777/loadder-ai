...
// visibleProductCount, productImageSize, saleEndsAt and saleLabel are
// deliberately siblings of productSettings, not fields inside it:
// productSettings is Commerce-adjacent protected truth in the V16 patch
// policy, so it can never be reached by a structured patch (Ask Loadder
// included). These are patch-reachable presentation levers over a products
// section's display and timing only — never price, stock or order truth.
export type SectionConfig = { id: string; type: "products" | "banner" | "trust" | "text" | "spacer" | "about" | "services" | "portfolio" | "team" | "text-image" | "cta" | "contact" | "category-grid" | "brand"; enabled: boolean; title: string; subtitle: string; body?: string; imageUrl?: string; ctaLabel?: string; ctaHref?: string; anchor?: string; navLabel?: string; showInNav?: boolean; columns?: number; mediaPosition?: "start" | "end"; items?: SectionItem[]; contact?: ContactConfig; backgroundColor: string; textColor: string; spacingTop: number; spacingBottom: number; productSettings?: ProductSettings; visibleProductCount?: number; productImageSize?: "regular" | "large"; saleEndsAt?: string; saleLabel?: string };
