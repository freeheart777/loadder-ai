...
function Header({ config, device, selected, select, interactive = true }: CanvasProps) {
...
function Hero({ config, device, selected, select, interactive = true }: CanvasProps) {
  ...
  return (
    ...
    >
      <div className="p-7 sm:p-12" style={{ textAlign: hero.alignment }}>
        ...
        <a href={hero.ctaHref || "#products"} onClick={interactive ? (event) => event.preventDefault() : undefined} className="mt-6 inline-flex min-h-12 items-center px-6 font-black" style={{ borderRadius: config.design.buttonRadius, background: config.design.primaryColor, color: "white" }}>{hero.ctaLabel}</a>
      </div>
...
function StorefrontCanvas(props: CanvasProps) {
  const { config, products, device, selected, select, interactive = true, onAddProduct, onReorderProduct } = props;
  return (
    <div style={{ background: config.design.backgroundColor, color: config.design.textColor }}>
      ...
      {config.sections.filter((section) => section.enabled).map((section) => {
        ...
        if (section.type === "products") {
          ...
          return (
            <EditorElement key={section.id} type="section" id={section.id} selected={selected} onSelect={select} interactive={interactive} style={{ background: section.backgroundColor, color: section.textColor, paddingTop: section.spacingTop, paddingBottom: section.spacingBottom }}>
              <div id="products" className="mx-auto px-4" style={{ maxWidth: config.design.containerWidth }}>
                ...
                <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${columns},minmax(0,1fr))` }}>
                  {shown.map((product) => <ProductCard key={product.id} product={product} settings={settings} config={config} selected={selected} select={select} compact={device === "mobile"} sectionId={section.id} interactive={interactive} onReorderProduct={onReorderProduct} />)}
                  {interactive && (
                    <button
                      type="button"
                      onClick={(event) => { event.stopPropagation(); onAddProduct?.(section.id); }}
                      className="grid min-h-52 place-items-center rounded-3xl border-2 border-dashed border-violet-300 bg-violet-50/70 p-5 text-center text-violet-700 transition hover:border-violet-500 hover:bg-violet-100"
                      aria-label="افزودن محصول"
                    >
                      <span><Plus size={34} weight="bold" className="mx-auto" /><b className="mt-3 block text-sm">افزودن محصول</b><small className="mt-1 block opacity-60">انتخاب از Catalog فروشگاه</small></span>
                    </button>
                  )}
                </div>
                {!shown.length && !interactive && <div className="grid min-h-52 place-items-center rounded-3xl border border-dashed text-sm opacity-45">هنوز محصولی برای این بخش انتخاب نشده است.</div>}
              </div>
            </EditorElement>
          );
        }
        return (
          <EditorElement key={section.id} type={type} id={section.id} selected={selected} onSelect={select} interactive={interactive} style={{ background: section.backgroundColor, color: section.textColor, paddingTop: section.spacingTop, paddingBottom: section.spacingBottom }}>
...
function CartCanvas(props: CanvasProps) {
  const { config, products, selected, select, device, interactive = true } = props;
  const items = products.slice(0, 2);
  const subtotal = items.reduce((sum, product) => sum + productView(product, config).regularPriceMinor, 0);
  return (
    <EditorElement type="cart" id="cart" selected={selected} onSelect={select} interactive={interactive} className="min-h-[720px] bg-slate-50 p-5 sm:p-8">
      <h2 className="text-2xl font-black">سبد خرید</h2>
      <div className={`mt-6 grid gap-5 ${device === "mobile" ? "" : "grid-cols-[1fr_320px]"}`}>
        <div className="space-y-3">{items.map((product) => { const view = productView(product, config); return <div key={product.id} className="flex items-center gap-3 rounded-2xl border bg-white p-3">{view.imageUrl ? <img src={view.imageUrl} alt={view.title} className="h-20 w-20 rounded-xl object-cover" /> : <div className="h-20 w-20 rounded-xl bg-slate-100" />}<div className="flex-1"><b>{view.title}</b><p className="mt-1 text-xs text-slate-400">تعداد: ۱</p></div><b>{formatMoney(view.regularPriceMinor, product.currency)}</b></div>; })}</div>
...
function CheckoutCanvas(props: CanvasProps) {
  const { config, selected, select, device, interactive = true } = props;
...
function SuccessCanvas(props: CanvasProps) {
  const { config, selected, select, interactive = true } = props;
  return <EditorElement type="success" id="success" selected={selected} onSelect={select} interactive={interactive} className="grid min-h-[720px] place-items-center bg-slate-50 p-6"><div className="max-w-md rounded-[32px] border bg-white p-9 text-center shadow-sm"><CheckCircle size={68} weight="fill" className="mx-auto" color={config.design.primaryColor}/><h2 className="mt-5 text-2xl font-black">{config.commerce.orderSuccessTitle}</h2><p className="mt-3 leading-7 text-slate-500">خلاصه سفارش و کد پیگیری از runtime واقعی نمایش داده خواهد شد.</p><button className="mt-6 min-h-12 px-6 font-black text-white" style={{ background: config.design.primaryColor, borderRadius: config.design.buttonRadius }}>بازگشت به فروشگاه</button></div></EditorElement>;
}
...
export default function StudioCanvas(props: CanvasProps) {
  ...
  return (
    <div className="mx-auto min-h-full transition-[width]" data-preview-device={props.device} data-canvas-interactive={props.interactive === false ? "false" : "true"} style={{ width, maxWidth: props.device === "desktop" ? "1200px" : undefined }}>
