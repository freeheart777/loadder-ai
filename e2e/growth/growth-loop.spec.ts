...
test.describe.serial('human-governed Growth Loop',()=>{
 ...
 test('completes canonical evidence, assessment and adoption without execution, then reloads',async({page},info)=>{
  ...
  await page.getByRole('button',{name:'پذیرش برای بررسی'}).click();await expect(page.getByText('تصمیم ثبت شد.')).toBeVisible();await expect(page.getByText('مجوز اجرای تبلیغ صادر نشده است.')).toBeVisible();
