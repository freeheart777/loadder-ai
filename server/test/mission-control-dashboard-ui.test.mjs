...
test("Mission Control Dashboard UI V1",async t=>{
  ...
  await t.test("keeps fact, belief, authority, UNKNOWN and explainability visually distinct",()=>{
    assert.match(mission,/واقعیت ·/);assert.match(mission,/برداشت پیشنهادی، نه واقعیت/);assert.match(mission,/رکورد معتبر دامنه/);assert.match(mission,/UNKNOWN ·/);assert.match(mission,/چرا اینجا نمایش داده شده/);
  });
  await t.test("preserves exact decision semantics and cautious S4 language",()=>{
  ...
  await t.test("renders reconciliation urgency and stale canonical reasons",()=>{
    assert.match(mission,/RECONCILIATION_REQUIRED/);assert.match(mission,/<Siren/);assert.match(mission,/staleReasons/);assert.match(mission,/\/dashboard\/business-brain/);
  });
  await t.test("isolates partial and total failures with retry and truthful empty state",()=>{
    assert.match(mission,/بخشی از سیگنال‌ها در دسترس نیست/);assert.match(mission,/هیچ سیگنالی قابل دریافت نیست/);assert.match(mission,/تلاش دوباره/);assert.match(mission,/مورد نیازمند توجهی دیده نمی‌شود/);assert.match(mission,/فقط بر پایه سیگنال‌های موفق فعلی/);
  });
