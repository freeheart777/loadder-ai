...
function fixture() {
  ...
  db.exec(`
    CREATE TABLE experiments(id TEXT PRIMARY KEY,workspace_id TEXT,status TEXT,ends_at TEXT,context_version_id TEXT,goal_ref TEXT);
    CREATE TABLE intelligence_recommendations(id TEXT PRIMARY KEY,workspace_id TEXT,recommendation_type TEXT,consideration_code TEXT,rationale_code TEXT,context_version_id TEXT,subject_type TEXT,subject_id TEXT,calculated_at TEXT,confidence_reason TEXT);
  ...
  const insertExperiment = (id, { workspace="a", end=ago(60), status="COMPLETED" }={}) => db.prepare("INSERT INTO experiments VALUES(?,?,?,?,?,?)").run(id,workspace,status,end,`context-${workspace}`,"/strategy/goals/0");
  const insertRecommendation = (id, { workspace="a", at=ago(30), subjectId="experiment-a", decided=false }={}) => {
    db.prepare("INSERT INTO intelligence_recommendations VALUES(?,?,?,?,?,?,?,?,?,?)").run(id,workspace,"EXPERIMENT_OUTCOME_REVIEW","GATHER_MORE_EVIDENCE","INCONCLUSIVE",`context-${workspace}`,"experiment",subjectId,at,"No causal confidence; private@example.com must not leak from another column.");
    if (decided) db.prepare("INSERT INTO decision_records VALUES(?,?,?)").run(`decision-${id}`,workspace,id);
  };
...
test("Mission Control V1", async t => {
  await t.test("signals are factual, bounded, deterministically ranked, and never serialize score", () => {
    const f=fixture();f.insertExperiment("experiment-a",{end:ago(60*24*8)});f.insertConversion("event-a");f.insertRecommendation("rec-a");f.insertCandidate("candidate-a",{state:"RECONCILIATION_REQUIRED",at:ago(1)});
  ...
  await t.test("S3 includes reconciliation and old pending but excludes recent pending",()=>{const f=fixture();f.insertCandidate("reconcile",{state:"RECONCILIATION_REQUIRED",at:ago(1)});f.insertCandidate("old",{at:ago(16)});f.insertCandidate("recent",{at:ago(14)});const ids=f.snapshot().items.filter(x=>x.signalId==="CONTENT_CANDIDATE_STUCK").map(x=>x.explainability.candidateId);assert.deepEqual(ids,["reconcile","old"]);assert.equal(MISSION_CONTROL_LIMITS.pendingAgeFloorMs,15*60*1000);f.db.close();});
