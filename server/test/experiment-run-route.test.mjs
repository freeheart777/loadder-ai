...
import { createExperimentRunsRouter } from "../app/routes/experiment-runs.mjs";

async function withServer(runService, fn) {
  const app = express();
  app.use(express.json());
  app.use("/api", createExperimentRunsRouter({ experimentRunService: runService }));
...
test("experiment run routes expose lifecycle operations", async () => {
  ...
  await withServer(runService, async (base) => {
    ...
    assert.equal(create.status, 201); assert.deepEqual((await create.json()).run, run);
    const list = await fetch(`${base}/api/experiments/exp-1/runs?status=RUNNING&limit=10`);
    assert.equal(list.status, 200); assert.deepEqual((await list.json()).runs, [run]);
    for (const [path, expectedStatus, expectedCall, body] of [["start", "RUNNING", "start", {}], ["complete", "COMPLETED", "complete", { outcome: { metric: 1 } }], ["fail", "FAILED", "fail", { outcome: { reason: "timeout" } }], ["cancel", "CANCELLED", "cancel", {}]]) {
      ...
      assert.equal(response.status, 200); assert.equal((await response.json()).run.status, expectedStatus); assert.equal(calls.at(-1)[0], expectedCall);
    }
  });
