...
test("worker does not execute an unclaimed run", async () => {
  let executions = 0;
  const worker = createExperimentRunWorker({ runService: { claim: () => null }, executor: async () => { executions += 1; return { ok: true }; } });
...
test("worker executes only after a successful claim and completes the run", async () => {
  ...
  const worker = createExperimentRunWorker({
    runService: {
      ...
      complete: (id, options) => { calls.push(["complete", id, options]); return { ...run, id, status: "COMPLETED" }; },
...
test("worker fails a claimed run after executor crash", async () => {
  let failed;
  const worker = createExperimentRunWorker({
    runService: { claim: () => ({ id: "run-1", contextVersionId: "ctx-1", status: "RUNNING" }), complete: () => { throw new Error("unexpected complete"); }, fail: (id, options) => { failed = { id, options }; return { id, status: "FAILED" }; } },
