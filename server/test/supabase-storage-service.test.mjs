...
test("storage service creates tenant-scoped signed upload URLs", async () => {
  ...
  const result=await service.createUploadUrl({workspaceId:"ws-1",siteProjectId:"project-1",filename:"لوگو اصلی.png",contentType:"image/png"});
  assert.equal(result.bucket,"site-assets");
  assert.equal(result.path,"ws-1/project-1/------.png");
...
test("storage service rejects traversal and can issue signed downloads", async () => {
  const service=createSupabaseStorageService({projectUrl:"https://example.supabase.co",serviceRoleKey:"service-key",fetchImpl:async(url)=>new Response(JSON.stringify({signedURL:"/object/sign/site-assets/ws-1/project-1/a.png?token=xyz"}),{status:200})});
