...
function setup(){
  ...
  const project=db.prepare("INSERT INTO business_builder_projects(id,workspace_id,name,status) VALUES(?,?,?,?)");
  project.run("p-disabled","w1","Disabled Commerce App","draft");
  project.run("p-active","w1","Live Commerce App","active");
  project.run("p-foreign","w2","Foreign App","active");
