...
const unifiedEditor = read("../../src/components/commerce/UnifiedProductMediaEditor.tsx");
...
test("legacy product editor retains URL and variant editing compatibility", () => {
...
test("active product media editor uses storage-backed gallery controls", () => {
  ...
  assert.doesNotMatch(unifiedEditor, /readAsDataURL|FileReader/);
});

test("variant/product editing contracts remain available while detail uses unified media", () => {
...
test("mobile controls remain touch-friendly and galleries adapt responsively", () => {
  assert.match(unifiedEditor, /grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4/);
  assert.match(unifiedEditor, /min-h-11/);
  assert.match(unifiedEditor, /overflow-x-auto/);
