...
test("site media falls back to local disk when Supabase is not configured", async () => {
  ...
  try {
    const storage = createSiteMediaStorageAdapter({ env: { SITE_MEDIA_LOCAL_DIR: dir, API_PORT: "3001" } });
    ...
    assert.match(publicUrl, /^http:\/\/localhost:3001\/api\/site-media-local\/object\//);
