...
const projectSections = (value) => Array.isArray(value) ? value.slice(0,100).map((item,index)=>{const source=object(item);return compact({
  id:`public-section-${index+1}`,type:string(source.type),enabled:boolean(source.enabled),title:string(source.title),subtitle:string(source.subtitle),imageUrl:string(source.imageUrl),ctaLabel:string(source.ctaLabel),backgroundColor:string(source.backgroundColor),textColor:string(source.textColor),spacingTop:number(source.spacingTop),spacingBottom:number(source.spacingBottom),productSettings:source.productSettings?projectProductSettings(source.productSettings):undefined,
});}) : undefined;
...
export function projectPublicStorePresentation(content) {
  const result={};
  ...
  if(Object.keys(v16).length)result.storeBuilderV16=compact({version:16,design:projectDesign(v16.design),header:projectHeader(v16.header),hero:projectHero(v16.hero),sections:projectSections(v16.sections),commerce:projectCommerce(v16.commerce)});
  return result;
}
