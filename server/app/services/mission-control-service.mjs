...
export function createMissionControlService({ repository, businessContextService, now = () => new Date() }) {
  function getMissionControl() {
    ...
    collect("S2", () => repository.closedExperimentsWithoutDecision(generatedAt, policy.maxItems), row => ({
      signalId: "EXPERIMENT_WINDOW_CLOSED_NO_DECISION", relevantAt: row.ends_at,
      score: 3 + overduePoints(row.ends_at, generated) + 2,
      facts: [fact("EXPERIMENT_STATUS", row.status, "EXPERIMENT", row.id), fact("MEASUREMENT_WINDOW_ENDED_AT", row.ends_at, "EXPERIMENT", row.id)],
    ...
    collect("S1", () => repository.undecidedRecommendations(policy.maxUndecidedRecommendations), row => ({
      signalId: "UNDECIDED_RECOMMENDATION", relevantAt: row.calculated_at, score: 2,
    ...
    candidates.sort((a, b) => b.score - a.score || Date.parse(a.relevantAt) - Date.parse(b.relevantAt) || a.signalId.localeCompare(b.signalId) || JSON.stringify(a.explainability).localeCompare(JSON.stringify(b.explainability)));
    if (candidates.length > policy.maxItems) truncated = true;
    const items = candidates.slice(0, policy.maxItems).map(({ score, relevantAt: _relevantAt, ...item }) => ({ ...item, band: missionControlBand(score) }));
    let banners = [];
    try {
