...
export class SiteProjectError extends Error {
  constructor(message, status = 400, code = "SITE_PROJECT_ERROR") {
    super(message);
    this.status = status;
    this.code = code;
  }
...
export function createSiteProjectService({ repository, businessContextService, now = () => new Date() }) {
  const requireType = (siteType) => {
    if (!TYPES.has(siteType)) throw new SiteProjectError("siteType is invalid.", 400, "SITE_TYPE_INVALID");
    return siteType;
  };
  ...
  function addAsset(id, input) {
    ...
    const asset = repository.addAsset(id, input, now().toISOString());
  ...
  function removeAsset(id) { if (!repository.removeAsset(id)) throw new SiteProjectError("Asset not found.", 404, "SITE_ASSET_NOT_FOUND"); return true; }
