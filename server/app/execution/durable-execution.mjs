    ...
    async run(request) {
      ...
      while (attempt < maxAttempts) {
        ...
        try {
          ...
          const failed = { ...running, status: attempt >= maxAttempts ? "failed" : "retryable", error: { name: error.name, code: error.code ?? "PROVIDER_ERROR", message: error.message }, updatedAt: now() };
          await store.save(failed);
          if (attempt >= maxAttempts) throw new DurableExecutionError("EXECUTION_FAILED", "execution exhausted retries", { executionId: request.executionId, attempt });
        }
      }
