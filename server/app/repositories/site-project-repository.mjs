...
const mapPublishVersion = (row) => row && ({
  id: row.id,
  workspaceId: row.workspace_id,
  siteProjectId: row.site_project_id,
  version: row.version,
...
export function createSiteProjectRepository(db) {
  ...
  function getPublished(id) {
    ...
    const version = getLatestPublishVersion(id);
    const assets = listAssets(id);
    return { project, version, assets };
  }
  ...
  function publish(id, now) {
    ...
    if (!current) return null;
    const ws = workspace();
    const nextVersion = db.prepare("SELECT COALESCE(MAX(version),0)+1 AS version FROM site_publish_versions WHERE site_project_id=? AND workspace_id=?").get(id, ws).version;
    const versionId = crypto.randomUUID();
    const manifest = {
      projectId: current.id,
      slug: current.slug,
      siteType: current.siteType,
      contextVersionId: current.contextVersionId,
      publishedAt: now,
      assetIds: listAssets(id).map((asset) => asset.id),
    };
    db.prepare("INSERT INTO site_publish_versions(id,workspace_id,site_project_id,version,context_version_id,content_json,manifest_json,published_at,created_at) VALUES(?,?,?,?,?,?,?,?,?)")
      .run(versionId, ws, id, nextVersion, current.contextVersionId, JSON.stringify(current.content), JSON.stringify(manifest), now, now);
    db.prepare("UPDATE site_projects SET status='PUBLISHED',published_at=COALESCE(published_at,?),updated_at=? WHERE id=? AND workspace_id=?")
      .run(now, now, id, ws);
    return get(id);
  }

  function listPublishVersions(siteProjectId) {
    return db.prepare("SELECT * FROM site_publish_versions WHERE site_project_id=? AND workspace_id=? ORDER BY version DESC").all(siteProjectId, workspace()).map(mapPublishVersion);
  }

  function getLatestPublishVersion(siteProjectId) {
    return mapPublishVersion(db.prepare("SELECT * FROM site_publish_versions WHERE site_project_id=? AND workspace_id=? ORDER BY version DESC LIMIT 1").get(siteProjectId, workspace()));
  }
  ...
  function addAsset(siteProjectId, { kind, name, url, storageKey = null, altText = null, metadata = {}, now }) {
    if (!get(siteProjectId)) return null;
    const id = crypto.randomUUID();
    db.prepare("INSERT INTO site_assets(id,workspace_id,site_project_id,kind,name,url,storage_key,alt_text,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)")
      .run(id, workspace(), siteProjectId, kind, name, url, storageKey, altText, JSON.stringify(metadata), now);
  ...
  return Object.freeze({ list, get, getPublished, create, update, publish, listPublishVersions, getLatestPublishVersion, remove, listAssets, addAsset, removeAsset });
}
