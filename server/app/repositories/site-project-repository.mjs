...
export function createSiteProjectRepository(db) {
  ...
  function rollbackPublishVersion(id, targetVersionId, now) { const ws = workspace(); return db.transaction(() => { const current = get(id); if (!current) return null; const target = mapPublishVersion(db.prepare("SELECT * FROM site_publish_versions WHERE id=? AND site_project_id=? AND workspace_id=?").get(targetVersionId, id, ws)); if (!target) return null; const nextVersion = db.prepare("SELECT COALESCE(MAX(version),0)+1 AS version FROM site_publish_versions WHERE site_project_id=? AND workspace_id=?").get(id, ws).version; const versionId = crypto.randomUUID(); const manifest = { ...target.manifest, projectId: id, contextVersionId: target.contextVersionId, publishedAt: now, rollbackOfVersionId: target.id }; db.prepare("INSERT INTO site_publish_versions(id,workspace_id,site_project_id,version,context_version_id,content_json,manifest_json,published_at,created_at) VALUES(?,?,?,?,?,?,?,?,?)").run(versionId, ws, id, nextVersion, target.contextVersionId, JSON.stringify(target.content), JSON.stringify(manifest), now, now); db.prepare("UPDATE site_projects SET context_version_id=?,content_json=?,status='PUBLISHED',published_at=COALESCE(published_at,?),updated_at=? WHERE id=? AND workspace_id=?").run(target.contextVersionId, JSON.stringify(target.content), now, now, id, ws); return { project: get(id), version: getLatestPublishVersion(id) }; })(); }
  ...
  const saveDraftWithRevision = db.transaction((id, { content, idempotencyKey, actorUserId = null, now }) => {
    const project = get(id);
    if (!project) return null;
    const { revision, created } = revisions.append({ siteProjectId: id, document: content, idempotencyKey, actorUserId });
    // A converged retry must not rewrite the current draft either.
    if (!created) return { project, revision, created };
    return { project: update(id, { content, now }), revision, created };
  });
  ...
  return Object.freeze({ list, get, getPublished, getPublishedPublic, getPublishedPublicByDomain, create, update, saveDraftWithRevision: (id, input) => saveDraftWithRevision(id, input), restoreDocumentRevision: (id, input) => restoreDocumentRevision(id, input), listDocumentRevisions, getDocumentRevision, currentDocumentRevision, publish, rollbackPublishVersion, listPublishVersions, getLatestPublishVersion, remove, listAssets, addAsset, removeAsset, createPreviewToken, revokePreviewToken, getPreviewByToken });
}
