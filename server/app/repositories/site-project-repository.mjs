...
export function createSiteProjectRepository(db) {
  ...
  // Public /s/:slug lookup: cross-workspace by design (the slug is globally
  // unique, migration 092) and published-only, exactly like getPublishedPublic.
  function getPublishedPublicBySlug(slug) { if (typeof slug !== "string" || !slug) return null; const row = db.prepare("SELECT id FROM site_projects WHERE slug=? AND status='PUBLISHED'").get(slug); return row ? getPublishedPublic(row.id) : null; }
  // Global slug existence check for collision-safe slug generation; returns only a boolean.
  function isSlugTaken(slug, excludeId = null) { return Boolean(db.prepare("SELECT 1 FROM site_projects WHERE slug=? AND id IS NOT ?").get(slug, excludeId)); }
  ...
  // Workspace of a project, for public routes that must enter its tenant context before any scoped read.
  function getProjectWorkspaceId(siteProjectId) { return db.prepare("SELECT workspace_id FROM site_projects WHERE id=?").get(siteProjectId)?.workspace_id ?? null; }
