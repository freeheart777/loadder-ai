...
export function createBusinessBuilderAdminHealth(db){
  ...
  return Object.freeze({summary(){
    ...
    const checks=[
      ...
      safeCount("commerceOutboxPending","SELECT COUNT(*) c FROM business_builder_commerce_outbox WHERE workspace_id=? AND status='pending' AND last_error IS NULL",w),
      safeCount("commerceOutboxRetrying","SELECT COUNT(*) c FROM business_builder_commerce_outbox WHERE workspace_id=? AND status='pending' AND last_error IS NOT NULL",w),
    ...
    const failedDeployments=by.deployments,pendingActions=by.actions,disabledAppUsers=by.appUsers,expiredInvites=by.invites,paymentEvents=by.payments,files=by.files,gitFailures=by.git,collaborationEvents=by.collaboration,commerceOutboxPending=by.commerceOutboxPending,commerceOutboxRetrying=by.commerceOutboxRetrying,commerceOutboxDelivered=by.commerceOutboxDelivered;
    const incidents=[...(Number(failedDeployments)>0?[{code:"DEPLOYMENT_FAILURE",severity:"high",count:failedDeployments}]:[]),...(Number(gitFailures)>0?[{code:"GIT_SYNC_FAILURE",severity:"medium",count:gitFailures}]:[]),...(Number(commerceOutboxRetrying)>0?[{code:"COMMERCE_OUTBOX_RETRYING",severity:"high",count:commerceOutboxRetrying}]:[]),...(Number(expiredInvites)>0?[{code:"EXPIRED_INVITES",severity:"low",count:expiredInvites}]:[])];
    const status=incidents.some(x=>x.severity==="high")?"degraded":incidents.length?"attention":unknownSources.length?"unknown":"healthy";
    return{status,incidents,unknownSources,counters:{failedDeployments,pendingActions,disabledAppUsers,expiredInvites,paymentEvents,files,gitFailures,collaborationEvents,commerceOutboxPending,commerceOutboxRetrying,commerceOutboxDelivered}};
  }});
}
