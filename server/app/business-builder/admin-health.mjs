...
export function createBusinessBuilderAdminHealth(db){
  ...
  return Object.freeze({summary(){
    const w=requireWorkspaceId(),at=new Date().toISOString(),staleClaimCutoff=new Date(Date.now()-60_000).toISOString();
    const checks=[
      ...
      safeCount("commerceOutboxClaimsActive","SELECT COUNT(*) c FROM business_builder_commerce_outbox WHERE workspace_id=? AND status='pending' AND dead_lettered_at IS NULL AND claim_token IS NOT NULL AND claim_expires_at > ?",w,at),
      safeCount("commerceOutboxClaimsStale","SELECT COUNT(*) c FROM business_builder_commerce_outbox WHERE workspace_id=? AND status='pending' AND dead_lettered_at IS NULL AND claim_token IS NOT NULL AND claim_expires_at <= ?",w,staleClaimCutoff)
    ];
    const by=Object.fromEntries(checks.map(x=>[x.source,x.value])),unknownSources=checks.filter(x=>!x.known).map(x=>x.source);
    const failedDeployments=by.deployments,pendingActions=by.actions,disabledAppUsers=by.appUsers,expiredInvites=by.invites,paymentEvents=by.payments,files=by.files,gitFailures=by.git,collaborationEvents=by.collaboration,commerceBindingsActive=by.commerceBindingsActive,commerceBindingsDisabled=by.commerceBindingsDisabled,commercePublishedStoresUnbound=by.commercePublishedStoresUnbound,commercePublishedStoresDisabled=by.commercePublishedStoresDisabled,commerceOutboxPending=by.commerceOutboxPending,commerceOutboxRetrying=by.commerceOutboxRetrying,commerceOutboxDeadLetter=by.commerceOutboxDeadLetter,commerceOutboxDelivered=by.commerceOutboxDelivered,commerceOutboxClaimsActive=by.commerceOutboxClaimsActive,commerceOutboxClaimsStale=by.commerceOutboxClaimsStale;
    const incidents=[...(Number(failedDeployments)>0?[{code:"DEPLOYMENT_FAILURE",severity:"high",count:failedDeployments}]:[]),...(Number(gitFailures)>0?[{code:"GIT_SYNC_FAILURE",severity:"medium",count:gitFailures}]:[]),...(Number(commercePublishedStoresUnbound)>0?[{code:"COMMERCE_STOREFRONT_UNBOUND",severity:"high",count:commercePublishedStoresUnbound}]:[]),...(Number(commercePublishedStoresDisabled)>0?[{code:"COMMERCE_STOREFRONT_BINDING_DISABLED",severity:"medium",count:commercePublishedStoresDisabled}]:[]),...(Number(commerceOutboxDeadLetter)>0?[{code:"COMMERCE_OUTBOX_DEAD_LETTER",severity:"high",count:commerceOutboxDeadLetter}]:[]),...(Number(commerceOutboxRetrying)>0?[{code:"COMMERCE_OUTBOX_RETRYING",severity:"high",count:commerceOutboxRetrying}]:[]),...(Number(commerceOutboxClaimsStale)>0?[{code:"COMMERCE_OUTBOX_STALE_CLAIM",severity:"medium",count:commerceOutboxClaimsStale}]:[]),...(Number(expiredInvites)>0?[{code:"EXPIRED_INVITES",severity:"low",count:expiredInvites}]:[])];
    const status=incidents.some(x=>x.severity==="high")?"degraded":incidents.length?"attention":unknownSources.length?"unknown":"healthy";
    return{status,incidents,unknownSources,counters:{failedDeployments,pendingActions,disabledAppUsers,expiredInvites,paymentEvents,files,gitFailures,collaborationEvents,commerceBindingsActive,commerceBindingsDisabled,commercePublishedStoresUnbound,commercePublishedStoresDisabled,commerceOutboxPending,commerceOutboxRetrying,commerceOutboxDeadLetter,commerceOutboxDelivered,commerceOutboxClaimsActive,commerceOutboxClaimsStale}};
  }});
}
