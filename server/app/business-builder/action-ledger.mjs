...
export class LoadderActionLedger{
  ...
  history(id){const workspaceId=requireWorkspaceId();if(!this.get(id))return[];return this.db.prepare("SELECT id,from_status,to_status,actor_id,result_json,created_at FROM business_builder_action_events WHERE workspace_id=? AND action_id=? ORDER BY created_at ASC,id ASC").all(workspaceId,id).map(r=>({id:r.id,fromStatus:r.from_status,toStatus:r.to_status,actorId:r.actor_id,result:parse(r.result_json),createdAt:r.created_at}));}
