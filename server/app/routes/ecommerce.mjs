...
export function createEcommerceRouter({ service, financialLedgerService = null, refundService = null }) {
  ...
  const handle = (error, res) => {
    ...
    if (String(error?.message || "").includes("UNIQUE constraint failed")) {
      return res.status(409).json({ success: false, code: "DUPLICATE_COMMERCE_VALUE", message: "A unique commerce value already exists." });
    }
  ...
  const run = (res, fn, status = 200) => { try { return res.status(status).json({ success: true, ...fn() }); } catch (error) { return handle(error, res); } };
  const requireFinancialAdmin = (req, res, next) => {
    const role = String(req.membership?.role || "").toLowerCase();
    if (!new Set(["owner", "admin"]).has(role)) return res.status(403).json({ success: false, code: "FINANCIAL_ADMIN_REQUIRED", message: "Owner or admin access is required for financial operations." });
    return next();
  };
  const finance = () => {
    if (!financialLedgerService) throw new FinancialLedgerError("Financial ledger service is unavailable.", "FINANCIAL_LEDGER_UNAVAILABLE", 503);
    return financialLedgerService;
  };
  const refunds = () => {
    if (!refundService) throw new RefundPersistenceError("Refund service is unavailable.", "REFUND_SERVICE_UNAVAILABLE", 503);
    return refundService;
  };
  const workspaceId = (req) => {
    const value = String(req.workspace?.id || "").trim();
    if (!value) throw new FinancialLedgerError("Workspace context is required for financial operations.", "FINANCIAL_WORKSPACE_REQUIRED", 403);
    return value;
  };
  ...
  router.get("/commerce/orders/:orderId/financials", requireFinancialAdmin, (req, res) => run(res, () => {
    const financials = finance().getOrderFinancials({ workspaceId: workspaceId(req), orderId: req.params.orderId, limit: req.query.limit || 100 });
    if (!financials) throw new FinancialLedgerError("Order not found.", "FINANCIAL_ORDER_NOT_FOUND", 404);
    return { financials };
  }));

  router.post("/commerce/orders/:orderId/financials/reconcile", requireFinancialAdmin, (req, res) => {
    try {
      const reconciliation = finance().reconcile({ workspaceId: workspaceId(req), orderId: req.params.orderId, userId: req.user?.id, actorRole: req.membership?.role });
      if (reconciliation.status === "missing_order") return res.status(404).json({ success: false, code: "FINANCIAL_ORDER_NOT_FOUND", message: "Order not found.", reconciliation });
      if (reconciliation.status === "not_paid") return res.status(409).json({ success: false, code: "FINANCIAL_ORDER_NOT_PAID", message: "Only paid orders can be reconciled into the capture ledger.", reconciliation });
      if (reconciliation.status === "conflict") return res.status(409).json({ success: false, code: "FINANCIAL_LEDGER_CONFLICT", message: "The immutable ledger conflicts with the current order payment snapshot.", reconciliation });
