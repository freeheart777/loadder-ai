...
import { createExperimentExecutionRouter } from "./experiment-execution.mjs";

export function createHumanGovernanceRouter({ service, experimentExecutionRouter } = {}) {
  const router = express.Router();
  const actor = (req) => ({ userId: req.user.id, membershipId: req.membership.id, role: req.membership.role });
  const handle = (error, res) => error instanceof HumanGovernanceError
  ...
  router.use(experimentExecutionRouter || createExperimentExecutionRouter());
  return router;
}
