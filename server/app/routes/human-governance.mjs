...
import { DecisionRecordReadError } from "../services/decision-record-read-service.mjs";

export function createHumanGovernanceRouter({ service, decisionRecordService }) {
  const router = express.Router();
  const actor = (req) => ({ userId: req.user.id, membershipId: req.membership.id, role: req.membership.role });
  const handle = (error, res) => error instanceof HumanGovernanceError || error instanceof DecisionRecordReadError
    ? res.status(error.status).json({ success: false, code: error.code, message: error.message })
    : res.status(500).json({ success: false, code: "HUMAN_GOVERNANCE_FAILED", message: "Human governance operation failed.", developmentDetail: process.env.NODE_ENV === "test" ? error.message : undefined });
  ...
  router.get("/intelligence/decisions/:decisionId/record", (req,res)=>{try{return res.json({success:true,record:decisionRecordService.getRecord(req.params.decisionId)});}catch(error){return handle(error,res);}});
