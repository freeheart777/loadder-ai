...
export function createContentItemRouter({ service }) {
  ...
  router.post("/content/items/from-asset", (request, response) => { try { const result = service.createFromAsset(request.body, actor(request), request.headers["idempotency-key"]); return response.status(result.reusedResult ? 200 : 201).json({ success: true, item: result.item, reusedResult: result.reusedResult }); } catch (error) { return handle(error, response); } });
