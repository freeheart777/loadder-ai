...
export function createIntegrationRouter({ service }) {
  ...
  const h = (e, s) => e instanceof IntegrationError || e?.code?.startsWith?.("SITE_") || e?.code?.startsWith?.("BUSINESS_CONTEXT_") ? s.status(e.status || 400).json({ success: false, code: e.code, message: e.message }) : s.status(500).json({ success: false, message: "Integration operation failed." });
  ...
  r.delete("/site-projects/:id/assets/:assetId", (q,s)=>{try{siteService.removeAsset(q.params.assetId);return s.json({success:true});}catch(e){return h(e,s);}});
  return r;
}
