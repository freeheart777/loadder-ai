...
function inspect(target) {
  ...
  if (target.includes("performance-observation") || target.includes(`${path.sep}performance${path.sep}`)) {
    const forbidden = [
      /from\s+["'][^"']*(ga4|google-analytics|google-ads|instagram|sms|crm|openai|provider-adapter|connector|fraud|analytics|campaign|website-builder|landing-builder|worker|queue|redis)[^"']*["']/i,
      /\b(customers|leads|marketing_campaigns|campaign_metrics|attribution_credit|fraud_score|ctr|cpa|roas|ranking)\b/i,
      /from\s+["'][^"']*db\/database\.mjs["']/,
      /\b(fetch|setInterval|setTimeout)\s*\(/,
    ];
  ...
  if (target.includes(`${path.sep}payments${path.sep}`) || target.includes("payment-order")) {
    const forbidden = [
      /from\s+["'][^"']*(openai|agent|embedding|execution|campaign|analytics|attribution|performance|crm|messaging|automation|worker|queue|redis|billing)[^"']*["']/i,
      /\b(attribution_touches|performance_observations|marketing_campaigns|campaign_metrics|execution_requests|customers|leads|refunds|shipments|inventory_reservations)\b/i,
      /from\s+["'][^"']*db\/database\.mjs["']/,
      /\b(setInterval|setTimeout)\s*\(/,
    ];
    if (forbidden.some((pattern) => pattern.test(source))) paymentBoundaryViolations.push(path.relative(serverRoot, target));
  }
  if (
    target.includes(`${path.sep}publishing${path.sep}`) ||
    target.includes("domain-publishing")
  ) {
    const forbidden = [
      /from\s+["'][^"']*(openai|agent|embedding|execution|campaign|analytics|attribution|performance|crm|messaging|automation|worker|queue|redis|billing|payment|shipping)[^"']*["']/i,
      /from\s+["'][^"']*db\/database\.mjs["']/,
      /\b(marketing_campaigns|campaign_metrics|execution_requests|payment_attempts|shipments|customers|leads)\b/i,
    ];
    if (forbidden.some((pattern) => pattern.test(source))) {
      domainPublishingBoundaryViolations.push(path.relative(serverRoot, target));
    }
  }
  if (target.includes("secure-forms-crm") || target.includes(`${path.sep}forms${path.sep}`)) {
    const forbidden = [
      /from\s+["'][^"']*(openai|agent|embedding|execution|campaign|automation|worker|queue|redis|billing|payment|shipping|connector|instagram|whatsapp|sms)[^"']*["']/i,
      /from\s+["'][^"']*db\/database\.mjs["']/,
      /\b(marketing_campaigns|campaign_metrics|execution_requests|customers|payment_attempts|shipments)\b/i,
...
if (performanceObservationBoundaryViolations.length) {
...
if (paymentBoundaryViolations.length) {
  console.error(`Payment boundary violations: ${paymentBoundaryViolations.join(", ")}`);
  process.exitCode = 1;
}
if (domainPublishingBoundaryViolations.length) {
  console.error(`Domain Publishing boundary violations: ${domainPublishingBoundaryViolations.join(", ")}`);
  process.exitCode = 1;
}
if (secureFormsCrmBoundaryViolations.length) {
